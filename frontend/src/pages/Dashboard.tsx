  ...
  return (
    <div className="flex flex-col min-h-screen">
                ...
                >
                  ...
                  <div className="flex justify-between items-end">
                    <div className="flex items-baseline gap-1">
                      <span
                        className={`font-headline-lg text-lg md:text-2xl font-bold font-technical-data ${kpi.text}`}
                      >
                        {kpi.value}
                      </span>

                      {kpi.subValue && (
                        <span className="text-[10px] text-status-warning/60 font-technical-data">
                          {kpi.subValue}
                        </span>
                      )}
                    </div>
                    ...
                    <MaterialIcon
                      name={kpi.icon}
                      className="text-primary-container/30 text-lg"
                    />
                  </div>
                </motion.div>
              </AnimatedCard>
      ...
      <section className="px-3 md:px-6 pb-6 grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 flex-1 min-h-0">
        ...
        <FadeIn delay={0.2}>
          <div className="lg:col-span-2 flex flex-col min-h-0 min-w-0">          <div className="flex justify-between items-center mb-3">
    ...
    <div className="flex-1 glass-panel p-3.5 font-technical-data text-[11px] overflow-y-auto space-y-3.5 custom-scrollbar">
      {agents.isLoading ? (
        ...
        decisions.map((d, i) => (
          <div
            key={d.id ?? i}
            className={`border-l-2 pl-3 py-1 ${d.agent_name?.toLowerCase().includes('risk')
          ...
          >
            <span className={`font-bold uppercase block mb-1 ${d.agent_name?.toLowerCase().includes('risk')
